...
1. 分析 HAR，先排除登入、付款、風控、通知及核心功能請求。
2. 依照上述優先次序選擇最簡單而有效的方法。
3. 修改前後檢查四個檔案有沒有重複處理。
4. 驗證語法、規則重複及非目標改動。
5. 將修改後的檔案同步到 `/Users/kewachan/Documents/github/Surge` 對應位置。
6. 只 stage 目標檔案，以實際修改內容撰寫簡短 commit message，然後 push；不要使用籠統的 `Updated by AI`。
